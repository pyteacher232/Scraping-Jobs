...
class MainScraper():
    def __init__(self):
        ...
        heading = [
            "nct_id", "sponsor or collaborator", "start_date"
        ]
    ...
    def get_details(self, url):
        ...
        for study in root.findall('study'):
            ...
            lead_sponsors = [e.text.strip() for e in study.findall('sponsors/lead_sponsor')]
            collaborators = [e.text.strip() for e in study.findall('sponsors/collaborator')]

            for sponsor in lead_sponsors+collaborators:
                result_row = [
                    nct_id, sponsor, start_date
                ]
                self.total_cnt += 1
